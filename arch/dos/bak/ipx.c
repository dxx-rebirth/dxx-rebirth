/* $Id: ipx.c,v 1.1.1.1 2006/03/17 19:52:52 zicodxx Exp $ */
/*
...
#ifdef RCS
static char rcsid[] = "$Id: ipx.c,v 1.1.1.1 2006/03/17 19:52:52 zicodxx Exp $";
#endif
...
//added on 10/29/98 by Kevin Bently to add ipx stuff
WORD ipx_num_outgoing_packets = 8;
ipx_packet * outpackets;
//end this section addition - Kevin Bently
...
//added/replaced on 10/29/98 by Kevin Bently for new ipx stuff
//-killed-        packets[0].ecb.socket_id = ipx_socket;
//-killed-        packets[0].ecb.fragment_count = 1;
//-killed-        packets[0].ecb.fragment_pointer[0] = DPMI_real_offset(&packets[0].ipx);
//-killed-        packets[0].ecb.fragment_pointer[1] = DPMI_real_segment(&packets[0].ipx);
//-killed-        packets[0].ipx.packet_type = 4;         // IPX packet
//-killed-        packets[0].ipx.destination.socket_id = ipx_socket;
//-killed-//      memcpy( packets[0].ipx.destination.network_id, &ipx_network, 4 );
//-killed-        memset( packets[0].ipx.destination.network_id, 0, 4 );
outpackets = dpmi_real_malloc( sizeof(ipx_packet)*ipx_num_packets, &ipx_packets_selector);

     for(i=0;i<ipx_num_outgoing_packets;i++)
      {
        outpackets[i].ecb.socket_id = ipx_socket;
        outpackets[i].ecb.fragment_count = 1;
        outpackets[i].ecb.fragment_pointer[0] = DPMI_real_offset(&outpackets[i].ipx);
        outpackets[i].ecb.fragment_pointer[1] = DPMI_real_segment(&outpackets[i].ipx);
        outpackets[i].ipx.packet_type = 4;              // IPX packet
        outpackets[i].ipx.destination.socket_id = ipx_socket;
        memset( outpackets[i].ipx.destination.network_id, 0, 4 );
      }
...
//added/replaced on 10/29/98 by Kevin Bently for new ipx stuff
//-killed-        // Make sure no one is already sending something
//-killed-        while( packets[0].ecb.in_use )
//-killed-        {
//-killed-        }
//-killed- 
//-killed-        if (packets[0].ecb.completion_code)     {
//-killed-                printf( "Send error %d for completion code\n", packets[0].ecb.completion_code );
//-killed-               //killed on 10/25/98 by Victor Rachels to fix LAN crash
//-killed-               //-killed- exit(1);
//-killed-               //end this section kill - Victor 
//-killed-        }
        while(!sendpacket)
         for(i=0;i<ipx_num_outgoing_packets;i++)
          if(!outpackets[i].ecb.in_use)
           sendpacket= &outpackets[i];
//end this section replacement - Kevin Bently

	// Fill in destination address
	if ( memcmp( network, &ipx_network, 4 ) )
                memcpy( sendpacket->ipx.destination.network_id, network, 4 );
	else
                memset( sendpacket->ipx.destination.network_id, 0, 4 );
        memcpy( sendpacket->ipx.destination.node_id.address, address, 6 );
        memcpy( sendpacket->ecb.immediate_address.address, immediate_address, 6 );
        sendpacket->pd.packetnum = ipx_packetnum++;

	// Fill in data to send
        sendpacket->ecb.fragment_size = sizeof(ipx_header) + sizeof(int) + datasize;

	assert( datasize > 1 );
        assert( sendpacket->ecb.fragment_size <= 576 );

        memcpy( sendpacket->pd.data, data, datasize );

	// Send it
        ipx_send_packet( &sendpacket->ecb );
