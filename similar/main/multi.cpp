...
void powerup_cap_state::cap_secondary_ammo(powerup_type_t type, uint8_t &player_ammo) const
{
	...
	player_ammo = capped;
	con_printf(CON_VERBOSE, "Capping secondary %u due to powerup cap: current=%u max=%u was=%u now=%hc", idx, get_current(idx), get_max(idx), static_cast<unsigned>(current_on_player), capped);
}
...
void change_playernum_to( int new_Player_num )
{
	...
	{
		Players[new_Player_num].callsign = get_local_player().callsign;
	}
