...
static int do_light_on(const d_delta_light_array &Delta_lights, const d_level_shared_destructible_light_state &LevelSharedDestructibleLightState, d_flickering_light_state &Flickering_light_state, const trigger &t)
{
	int ret=0;
	const auto op = [&Delta_lights, &LevelSharedDestructibleLightState, &Flickering_light_state, &ret](const vmsegptridx_t segnum, const unsigned sidenum) {
			...
			if (TmapInfo[segnum->unique_segment::sides[sidenum].tmap_num2 & 0x3fff].lighting) {
				ret |= add_light(Delta_lights, LevelSharedDestructibleLightState, segnum, sidenum);		//any light sets flag
...
//would not be if they had previously been shot out).
static int do_light_off(const d_delta_light_array &Delta_lights, const d_level_shared_destructible_light_state &LevelSharedDestructibleLightState, d_flickering_light_state &Flickering_light_state, const trigger &t)
{
	int ret=0;
	const auto op = [&Delta_lights, &LevelSharedDestructibleLightState, &Flickering_light_state, &ret](const vmsegptridx_t segnum, const unsigned sidenum) {
			...
			if (TmapInfo[segnum->unique_segment::sides[sidenum].tmap_num2 & 0x3fff].lighting) {
				ret |= subtract_light(Delta_lights, LevelSharedDestructibleLightState, segnum, sidenum);	//any light sets flag
...
window_event_result check_trigger_sub(object &plrobj, const trgnum_t trigger_num, const playernum_t pnum, const unsigned shot)
{
	...
	switch (trigger.type)
	{
		...
		case TT_LIGHT_OFF:
			if (do_light_off(Delta_lights, LevelSharedDestructibleLightState, Flickering_light_state, trigger))
				print_trigger_message(pnum, trigger, shot, "Light%s off!");
			break;

		case TT_LIGHT_ON:
			if (do_light_on(Delta_lights, LevelSharedDestructibleLightState, Flickering_light_state, trigger))
