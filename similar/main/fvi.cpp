...
int find_vector_intersection(const fvi_query &fq, fvi_info &hit_data)
{
	...
	hit_type = fvi_sub(hit_pnt, hit_seg2, *fq.p0, vcsegptridx(fq.startseg), *fq.p1, fq.rad, imobjptridx(fq.thisobjnum), fq.ignore_obj_list, fq.flags, hit_data.seglist, segment_exit, visited, fvi_hit_side, fvi_nest_count, fvi_hit_pt_seg, wall_norm);
	segnum_t hit_seg;
...
//MATT: TAKE OUT THIS HACK AND FIX THE BUGS!
	...
	if (hit_seg == segment_none) {
		...
		new_hit_type = fvi_sub(new_hit_pnt, new_hit_seg2, *fq.p0, vcsegptridx(fq.startseg), *fq.p1, 0, imobjptridx(fq.thisobjnum), fq.ignore_obj_list, fq.flags, hit_data.seglist, segment_exit, visited, fvi_hit_side, fvi_nest_count, fvi_hit_pt_seg, wall_norm);
					...
					if (face_hit_type) {            //through this wall/door
						...
						if ((wid_flag & WID_FLY_FLAG) ||
							...
							if (!visited[newsegnum]) {                //haven't visited here yet
								...
								sub_hit_type = fvi_sub(sub_hit_point, sub_hit_seg, p0, startseg.absolute_sibling(newsegnum), p1, rad, thisobjnum, ignore_obj_list, flags, temp_seglist, startseg, visited, fvi_hit_side, fvi_nest_count, fvi_hit_pt_seg, wall_norm);
