...
#ifdef RCS
static char rcsid[] = "$Id: config.c,v 1.1.1.1 2006/03/17 19:55:48 zicodxx Exp $";
#endif
...
/*	joy_get_cal_vals(joy_axis_min, joy_axis_center, joy_axis_max);
	...
	for (i=0;i<4;i++)
   {
	...
	PHYSFSX_puts(infile, str);
	sprintf (str, "%s=%d\n", redbook_enabled_str, !GameArg.SndEnableRedbook?save_redbook_enabled:Redbook_enabled);
	PHYSFSX_puts(infile, str);
...
PrefsInfoHandle		piHdl;
	...
	StringPtr			app_string;
		...
		//	Set default to '�pplication�Prefs', PREF 0
