...
void sb_show_score()
{
	...
	x = HUD_SCALE_X(SB_SCORE_RIGHT)-w-LHX(2);
...
void sb_show_score_added()
{
	...
	char	score_str[32];
	int x;
	static int last_x[2]={SB_SCORE_RIGHT_L,SB_SCORE_RIGHT_H};
	...
	if (score_time > 0) {
		...
		x = HUD_SCALE_X(SB_SCORE_ADDED_RIGHT)-w-LHX(2);
...
void hud_show_orbs (void)
{
	if (Game_mode & GM_HOARD) {
		...
		if (Cockpit_mode == CM_FULL_COCKPIT) {
			y = HUD_SCALE_Y(2*Line_spacing);
			x = HUD_SCALE_X(4*GAME_FONT->ft_w);
		}
...
void draw_weapon_info_sub(int info_index,gauge_box *box,int pic_x,int pic_y,char *name,int text_x,int text_y)
{
	...
	if (GameArg.GfxGaugeHudMode == 0) {
		...
		if (info_index == LASER_ID || info_index == SUPER_LASER_ID) {
			...
			gr_printf(text_x,text_y+HUD_SCALE_Y(Line_spacing), temp_str);
	
			if (Players[Player_num].flags & PLAYER_FLAGS_QUAD_LASERS) {
				strcpy(temp_str, TXT_QUAD);
				gr_printf(text_x,text_y+HUD_SCALE_Y(2*Line_spacing), temp_str);
