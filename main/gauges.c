...
void hud_show_keys(void)
{
	...
	int y=HUD_SCALE_Y(GameBitmaps[ GET_GAUGE_INDEX(GAUGE_LIVES) ].bm_h+2)+FSPACY(1);
...
void hud_show_orbs (void)
{
	if (Game_mode & GM_HOARD) {
		...
		hud_bitblt(x,y,bm);
		gr_printf(x+HUD_SCALE_X(bm->bm_w), y, " x %d", Players[Player_num].secondary_ammo[PROXIMITY_INDEX]);
	}
}
...
void hud_show_flag(void)
{
	if ((Game_mode & GM_CAPTURE) && (Players[Player_num].flags & PLAYER_FLAGS_FLAG)) {
		...
		PAGE_IN_GAUGE( icon );
		hud_bitblt(x,y,&GameBitmaps[ GET_GAUGE_INDEX(icon) ]);
...
void hud_show_weapons(void)
{
	...
	if (PlayerCfg.HudMode==1){
		...
		hud_show_weapons_mode(0,1,2,x1+FSPACX(12),y);
		hud_show_weapons_mode(1,1,1,x2,y);
		hud_show_weapons_mode(1,1,2,x2+FSPACX(12),y);
...
void init_gauges()
{
	...
	gr_free_sub_bitmap(WinBoxOverlay[0]);
	gr_free_sub_bitmap(WinBoxOverlay[1]);
	WinBoxOverlay[0] = gr_create_sub_bitmap(&GameBitmaps[cockpit_bitmap[PlayerCfg.CockpitMode+(HIRESMODE?(Num_cockpits/2):0)].index],(PRIMARY_W_BOX_LEFT),(PRIMARY_W_BOX_TOP),(PRIMARY_W_BOX_RIGHT-PRIMARY_W_BOX_LEFT+2),(PRIMARY_W_BOX_BOT-PRIMARY_W_BOX_TOP+2));
	WinBoxOverlay[1] = gr_create_sub_bitmap(&GameBitmaps[cockpit_bitmap[PlayerCfg.CockpitMode+(HIRESMODE?(Num_cockpits/2):0)].index],(SECONDARY_W_BOX_LEFT),(SECONDARY_W_BOX_TOP),(SECONDARY_W_BOX_RIGHT-SECONDARY_W_BOX_LEFT+2),(SECONDARY_W_BOX_BOT-SECONDARY_W_BOX_TOP+2));
}
...
void show_reticle()
{
...
#endif
	{
		...
		hud_bitblt(x+HUD_SCALE_X(cross_offsets[ofs].x),y+HUD_SCALE_Y(cross_offsets[ofs].y),&GameBitmaps[GET_GAUGE_INDEX(gauge_index)] );
	
		gauge_index = RETICLE_PRIMARY + primary_bm_num;
		PAGE_IN_GAUGE( gauge_index );
		hud_bitblt(x+HUD_SCALE_X(primary_offsets[ofs].x),y+HUD_SCALE_Y(primary_offsets[ofs].y),&GameBitmaps[GET_GAUGE_INDEX(gauge_index)] );
	
		gauge_index = RETICLE_SECONDARY + secondary_bm_num;
		PAGE_IN_GAUGE( gauge_index );
		hud_bitblt(x+HUD_SCALE_X(secondary_offsets[ofs].x),y+HUD_SCALE_Y(secondary_offsets[ofs].y),&GameBitmaps[GET_GAUGE_INDEX(gauge_index)] );
	}
}
