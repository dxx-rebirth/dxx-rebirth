...
*/
/*
 * $Source: /cvsroot/dxx-rebirth/d1x-rebirth/main/titles.c,v $
 * $Revision: 1.2 $
 ...
 *
 *
 * Revision 3.0   1998/02/21  08:15:35	Tim Singer
 * Made briefing sequences support more colors with $C
 * and the dollar sign and semicolon with $$ and $;
 * ($; only is needed if you have one at the beginning of a line.
 *  otherwise, use ;)
 *
...
if (grd_curcanv->cv_bitmap.bm_h > 320 && cfexist(DESCENT_DATA_PATH "brief01h.pcx")) // ZICO - use hires if available
{
	for (i = 0; i < MAX_BRIEFING_SCREEN; i++)	{
		...
		if (Briefing_screens_h[i].level_num == level_num )	{
			found_level=1;
			last_level = i;
		}
	}
...
char	Bitmap_name[32] = "";
#define	EXIT_DOOR_MAX	0 //removed door animation becaused broken in OGL - zico 20051015
...
//                        vfx_set_palette_sub( New_pal );
	...
	if (grd_curcanv->cv_bitmap.bm_h > 320 && cfexist(DESCENT_DATA_PATH "brief01h.pcx")) // ZICO - we use hires so we move the exit door away because we have it in the bitmap - very dirty :)
	{
		switch (Animating_bitmap_type) {
			case 0:	bitmap_canv = gr_create_sub_canvas(grd_curcanv, rescale_x(9999), rescale_y(9999), 64, 64);	break;
			case 1:	bitmap_canv = gr_create_sub_canvas(grd_curcanv, rescale_x(9999), rescale_y(9999), 94, 94);	break;	//	Adam: Change here for your new animating bitmap thing. 94, 94 are bitmap size.
...
void show_briefing_bitmap(grs_bitmap *bmp)
{
	...
	if (grd_curcanv->cv_bitmap.bm_h > 320 && cfexist(DESCENT_DATA_PATH "brief01h.pcx")) // ZICO - use hires if available
	{
		bitmap_canv = gr_create_sub_canvas(grd_curcanv, -99, -99, 166, 138); // ZICO move dravis' face away because we already have it in the bitmap - very dirty :)
	} else {
		if (grd_curcanv->cv_bitmap.bm_h > 320) { // ZICO - added to take dravis' face on a better place it res > 320x***
			bitmap_canv = gr_create_sub_canvas(grd_curcanv, 500, 180, 166, 138);
		} else {
...
//	Robot_angles.h += 0;
	...
	int x = rescale_x(138);
	int y = rescale_y(55);
	int w = rescale_x(162); // ZICO - old value 166 - fixes overlay with hires screens
	int h = rescale_y(134); // ZICO - old value 138 - fixes overlay with hires screens
