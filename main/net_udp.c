...
// Send PID_ENDLEVEL in regular intervals and listen for them
...
// Same as above but used when player pressed ESC during kmatrix
...
void net_udp_send_game_info(struct _sockaddr sender_addr, ubyte info_upid)
{
	...
	if (info_upid == UPID_GAME_INFO_LITE)
	{
		...
		buf[0] = info_upid;														len++;
		PUT_INTEL_INT(buf + len, D2X_IVER);  									len += 4;
...
void net_udp_process_game_info(ubyte *data, int data_len, struct _sockaddr game_addr, int lite_info)
{
	...
	if (lite_info)
	{
		...
		if (recv_game.program_iver != D2X_IVER)
			return;
...
GetPlayersAgain:
	j=newmenu_do1( NULL, title, MAX_PLAYERS+4, m, net_udp_start_poll, 1 );
