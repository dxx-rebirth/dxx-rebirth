...
int udp_tracker_unregister()
{
	...
	// Send it off
	return sendto( UDP_Socket[2], pBuf, iLen, 0, (struct sockaddr *)&TrackerSocket, sizeof( TrackerSocket ) );
}
...
void net_udp_send_game_info(struct _sockaddr sender_addr, ubyte info_upid)
{
	...
	if (info_upid == UPID_GAME_INFO_LITE)
	{
		...
		PUT_INTEL_SHORT(buf + len, D2XMAJORi); 						len += 2;			// 3
		PUT_INTEL_SHORT(buf + len, D2XMINORi); 						len += 2;			// 5
		PUT_INTEL_SHORT(buf + len, D2XMICROi); 						len += 2;			// 7
